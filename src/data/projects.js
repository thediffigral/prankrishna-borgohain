export const projects = [
  {
    slug: 'aakhar',
    year: 2014,
    title: 'Aakhar',
    subtitle: 'A Unicode Assamese e-magazine',
    excerpt: 'An Assamese e-magazine born from Room 101 of the New Boys\' Hostel at North Lakhimpur College, at a time when Unicode publishing in Assamese was still finding its feet.',
    description: 'Aakhar began with my roommate Jyotishman and me. He was the editor and I worked with him as an associate. We wanted to make a magazine entirely in Unicode Assamese. We were college students, working from our hostel room, and the project felt like a small but bold contribution to the emerging world of Unicode Assamese literature.',
    role: 'Associate to the editor',
    collaborators: 'Jyotishman — Editor',
    type: 'E-magazine'
  },
  {
    slug: 'awrr',
    year: 2015,
    title: 'Awrr',
    subtitle: 'A rock album from a small town',
    excerpt: 'A rock-band project that began on the verandah of my home and grew into an album that was heard, loved and appreciated by many.',
    description: 'We came from a small town and formed a rock band. For young people like us, making an album was the kind of dream that could easily remain a dream. Awrr started on the verandah of my home and gradually became a real album, made with the people who shared that ambition.',
    role: 'Band member and co-creator',
    collaborators: 'The members of Awrr',
    type: 'Music'
  },
  {
    slug: 'the-indian-dog-blog',
    year: 2017,
    title: 'The Indian Dog Blog',
    subtitle: 'A project about dogs and their people',
    excerpt: 'A project that began with my pet dog Aditya and grew from funny dog dialogues into a place where people shared stories about their pets.',
    description: 'The Indian Dog Blog was dedicated to my beloved pet dog Aditya. It began as Dog-Talks, where I added funny imagined dialogues to photographs of dogs. Later, I turned it into an Instagram page where different people could share their own experiences with their pets.',
    role: 'Creator',
    collaborators: 'Contributors and pet owners from the community',
    type: 'Instagram project'
  },
  {
    slug: '3-mahapurux',
    year: 2020,
    title: '3 Mahapurux',
    subtitle: 'An Assamese comic series for Instagram',
    excerpt: 'An online Assamese comic series about the everyday lives of three fictional characters, created with Tonmoy and brought to life through Rahul\'s drawings.',
    description: 'Tonmoy and I thought up 3 Mahapurux as an online Assamese comic series. Rahul helped us with his amazing drawing work. We published stories from the everyday lives of three fictional characters on Instagram, building a small recurring world one story at a time.',
    role: 'Co-creator',
    collaborators: 'Tonmoy — Co-creator; Rahul — Illustrator',
    type: 'Comic series'
  },
  {
    slug: 'no-mercy-the-battle-of-badminton',
    year: 2023,
    title: 'No Mercy — The Battle of Badminton',
    subtitle: 'Badminton, played a little differently',
    excerpt: 'A badminton club built around unusual rules, where the game became something closer to playing chess with a racket.',
    description: 'No Mercy began as a badminton club that Sritam and I co-founded. We developed a set of unusual badminton rules that changed the character of the game, making it feel almost like playing chess in badminton. Research scholars and other enthusiasts around us contributed equally to the project.',
    role: 'Co-founder',
    collaborators: 'Sritam — Co-founder; research scholars and other enthusiasts',
    type: 'Club / Game'
  },
  {
    slug: 'etyadi-films',
    year: 2024,
    title: 'Etyadi Films',
    subtitle: 'Turning imagination into moving images',
    excerpt: 'A filmmaking project shaped with friends who helped turn ideas into short films, advertisements and a web series.',
    description: 'Some enthusiastic friends of mine are brilliant actors, and they helped me turn my imagination into reality. Through Etyadi Films I have made short films, advertisements—including an advertisement that won a local competition prize—and a web series. Of all of it, my heart is most into making advertisements.',
    role: 'Creator',
    collaborators: 'Friends, actors and collaborators across the projects',
    type: 'Films / Studio'
  }
];
